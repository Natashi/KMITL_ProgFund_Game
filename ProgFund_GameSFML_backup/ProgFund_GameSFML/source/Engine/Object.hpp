...
public:
	...
	static XMMATRIX CreateWorldMatrix2D(Vector3f* const position, Vector3f* const angle,
		Vector3f* const scale, XMMATRIX* const camera);
	static XMMATRIX CreateWorldMatrix2D(Vector3f* const position, Vector2f* const angleX,
		Vector2f* const angleY, Vector2f* const angleZ, Vector3f* const scale, XMMATRIX* const camera);

	virtual void SetPosition(float x, float y, float z) { position_ = Vector3f(x, y, z); }
	virtual void SetPosition(const Vector3f& pos) { position_ = pos; }
	...
	virtual void SetAngle(const Vector3f& pos) { 
	...
	virtual void SetScale(float x, float y, float z) { scale_ = Vector3f(x, y, z); }
	virtual void SetScale(const Vector3f& pos) { scale_ = pos; }
	...
	virtual void SetColor(DWORD rgb) { color_ = (rgb & 0x000000ff) | (color_ & 0xffffff00); }
	virtual void SetColor(byte r, byte g, byte b) { SetColor(GenColorXRGB(r, g, b)); }
	virtual void SetAlpha(byte alpha) { color_ = ((DWORD)alpha << 24) | (color_ & 0xffffff00); }
	/*
...
protected:
	Vector3f position_;
	Vector3f angle_;
	Vector2f angleX_;	//[cos, sin]
	Vector2f angleY_;	//[cos, sin]
	Vector2f angleZ_;	//[cos, sin]
	Vector3f scale_;
	DWORD color_;
...
public:
	...
	shared_ptr<DataBuffer> GetVertexBuffer() { return bufferVertex_; }
	shared_ptr<DataBuffer> GetIndexBuffer() { return bufferIndex_; }
protected:
	shared_ptr<DataBuffer> bufferVertex_;
	shared_ptr<DataBuffer> bufferIndex_;
};
...
public:
	...
	void SetSourceRectNormalized(const GLRect& rc);
	void SetSourceRect(const GLRect& rc);
	void SetDestRect(const GLRect& rc);
	void SetDestCenter();
